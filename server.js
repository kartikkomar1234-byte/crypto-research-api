...
async function loadAllPairs(){
  ...
  try{
    ...
    tickers.forEach(t => {
      ...
      if(market.endsWith('INR') && parseFloat(t.last_price||0) > 0){
        // Extract symbol: remove INR suffix
        // Handle cases like BTCINR -> BTC, 1INCHINR -> 1INCH
        const sym = market.replace(/INR$/, '');
        if(sym && sym.length > 0 && sym.length <= 15){
          pairs[sym] = {
            ticker: market,
            candle: `B-${sym}_INR`,
            name: sym, // will be enriched later if needed
          };
        }
      }
    });
